...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (empty($from_location) || empty($to_location)) {
        ...
        $check_sql = "SELECT id FROM fare_chart WHERE `from` = ? AND `to` = ?";
        $check_stmt = $conn->prepare($check_sql);
        $check_stmt->bind_param("ss", $from_location, $to_location);
        $check_stmt->execute();
        ...
        if ($check_result->num_rows > 0) {
            ...
            // Insert new fare
            $insert_sql = "INSERT INTO fare_chart (`from`, `to`, fare, distance_km, operating_bus) VALUES (?, ?, ?, ?, ?)";
            $insert_stmt = $conn->prepare($insert_sql);
            $insert_stmt->bind_param("ssdds", $from_location, $to_location, $fare, $distance, $operating_bus);

            if ($insert_stmt->execute()) {
                // Log activity
                $log_sql = "INSERT INTO admin_logs (admin_id, action, details, ip_address) VALUES (?, 'insert', ?, ?)";
                $log_stmt = $conn->prepare($log_sql);
                $details = "নতুন ভাড়া যোগ করা হয়েছে: {$from_location} → {$to_location} (ভাড়া: ৳{$fare}, দূরত্ব: {$distance}কিমি)";
                ...
                $history_sql = "INSERT INTO fare_update_history (admin_id, update_type, records_affected) VALUES (?, 'single', 1)";
                $history_stmt = $conn->prepare($history_sql);
                $history_stmt->bind_param("i", $_SESSION['admin_id']);
                $history_stmt->execute();
                $history_stmt->close();

                $success = "ভাড়ার তথ্য সফলভাবে যোগ করা হয়েছে!";
                
                // Clear form
                $_POST = array();
            } else {
                $error = "ভাড়ার তথ্য যোগ করতে ব্যর্থ হয়েছে!";
            }
            $insert_stmt->close();
        }
        $check_stmt->close();
...
<head>
    ...
    <title>নতুন ভাড়া যোগ - SafeRideBD অ্যাডমিন</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <ul class="nav-menu">
            ...
            <li class="nav-item">
                <a href="admin_add_fare.php" class="nav-link active">
    ...
    <div class="main-content">
        <div class="page-header">
            <h1 class="page-title">নতুন ভাড়া যোগ করুন</h1>
            <p class="page-subtitle">নতুন রুটের জন্য ভাড়ার তথ্য যুক্ত করুন</p>
        </div>
        ...
        <div class="form-card">
            <form method="POST" action="admin_add_fare.php" id="fareForm">
                <div class="form-row">
                ...
                <div class="form-row">
                    <div class="form-group">
                        ...
                        <input type="number" name="fare" class="form-control" 
                               ...
                               value="<?php echo isset($_POST['fare']) ? htmlspecialchars($_POST['fare']) : ''; ?>">
                    </div>
                ...
                <div class="form-group">
                    ...
                    <textarea name="operating_bus" class="form-control" 
                              placeholder="যেমন: ২, ৩, ৪, ৫, ৬, ৭"><?php echo isset($_POST['operating_bus']) ? htmlspecialchars($_POST['operating_bus']) : ''; ?></textarea>
                    <div class="info-text">
